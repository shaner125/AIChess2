...
*/
    ...
    private int AssessValue(Square currentSquare) {
        int value;
        String tmp = returnName(currentSquare.getXC()*75, currentSquare.getYC()*75);
        if(tmp.contains("Queen")){
            value = 9;
        }
        else if(tmp.contains("King")){
            value = 10;
        }
        else if(tmp.contains("Rook")){
            value = 5;
        }
        else if(tmp.contains("Bishop")){
            value = 3;
        }
        else if(tmp.contains("Pawn")){
            value = 1;
        }
        else if(tmp.contains("Knight")){
            value = 3;
        }
        else{
            value =0;
  ...
  if(startingSquare.getYC() == 1){
        ...
        validM = new Move(startingSquare, tmp, AssessValue(tmp));
        validM2 = new Move(startingSquare, tmp1, AssessValue(tmp1));
        validM3 = new Move(startingSquare, tmp2, AssessValue(tmp2));
        validM4 = new Move(startingSquare, tmp3, AssessValue(tmp3));
     ...
     if(!piecePresent(((tmp2.getXC()*75)+20), (((tmp2.getYC()*75)+20))) && !piecePresent(((tmp3.getXC()*75)+20), (((tmp3.getYC()*75)+20)))){
      ...
      validM = new Move(startingSquare, tmp1, AssessValue(tmp1));
      validM2 = new Move(startingSquare, tmp2, AssessValue(tmp2));
      validM3 = new Move(startingSquare, tmp, AssessValue(tmp));
...
//  Stack bishop = new Stack();
//  Stack rook = new Stack();
//  Stack queen = new Stack();
//  Stack pawn = new Stack();
...
//      while(!tempK.empty()){
//        Move tempKnight = (Move)tempK.pop();
//        knight.push(tempKnight.getLanding());
//      }
//      orderMoves(knight);
//    }
...
/*
  ...
  that returns a Boolean points to let us know if a supplied square will be adajacent to another square with a
...
*/
  if(!((tmpx1 > 7))){
    /*
      ...
      a condition here to check that the new points of x (tmpx1) is not greater than 7.
      ...
      As the unchanged y points is already a location on the board we don't need to check the location and can simply
      make a call to checkSurroundingSquares for this new Square.

      If checkSurroundingSquares returns a positive points we jump inside the condition below:
    ...
    if(!(tmpy2 < 0)){
      if(checkSurroundingSquares(tmp2)){
        ...
        if(!piecePresent(((tmp2.getXC()*75)+20), (((tmp2.getYC()*75)+20)))){
          ...
          System.out.println("The values that we are going to be looking at are : "+((tmp2.getXC()*75)+20)+" and the y points is : "+((tmp2.getYC()*75)+20));
  ...
  if(!((tmpx2 < 0))){
    ...
    if(checkSurroundingSquares(tmp3)){
      validM = new Move(startingSquare, tmp3, AssessValue(tmp3));
    ...
    if(!(tmpy1 > 7)){
      if(checkSurroundingSquares(tmp4)){
        validM2 = new Move(startingSquare, tmp4, AssessValue(tmp4));
    ...
    if(!(tmpy2 < 0)){
      if(checkSurroundingSquares(tmp5)){
        validM3 = new Move(startingSquare, tmp5, AssessValue(tmp5));
  ...
  if(!(tmpy1 > 7)){
    if(checkSurroundingSquares(tmp7)){
      validM2 = new Move(startingSquare, tmp7, AssessValue(tmp7));
  ...
  if(!(tmpy2 < 0)){
    if(checkSurroundingSquares(tmp8)){
      validM3 = new Move(startingSquare, tmp8, AssessValue(tmp8));
...
/*
  ...
  own piece. As seen in the below grid the Rook can either move in a horizontal direction (x changing points)
...
private Stack getRookMoves(int x, int y, String piece){
  ...
  /*
    There are four possible directions that the Rook can move to:
      - the x points is increasing
      - the x points is decreasing
      - the y points is increasing
      - the y points is decreasing

    Each of these movements should be catered for. The loop guard is set to incriment up to the maximun number of squares.
    On each iteration of the first loop we are adding the points of i to the current x coordinate.
  ...
  */
  ...
  for(int k=1;k < 8;k++){
    ...
    if(!(tmpy3 > 7 || tmpy3 < 0)){
      ...
      validM3 = new Move(startingSquare, tmp3, AssessValue(tmp3));
      if(!piecePresent(((tmp3.getXC()*75)+20), (((tmp3.getYC()*75)+20)))){
        moves.push(validM3);
      }
      else{
  ...
  for(int l=1;l < 8;l++){
    ...
    if(!(tmpy4 > 7 || tmpy4 < 0)){
      ...
      validM4 = new Move(startingSquare, tmp4, AssessValue(tmp4));
      if(!piecePresent(((tmp4.getXC()*75)+20), (((tmp4.getYC()*75)+20)))){
        moves.push(validM4);
      }
      else{
  ...
  */
  ...
  for(int k=1;k < 8;k++){
    ...
    if(!(tmpk > 7 || tmpk < 0 || tmpy2 > 7 || tmpy2 < 0)){
      ...
      validM2 = new Move(startingSquare, tmpK1, AssessValue(tmpK1));
  ...
  for(int l=1;l < 8;l++){
    ...
    if(!(tmpL2 > 7 || tmpL2 < 0 || tmpy3 > 7 || tmpy3 < 0)){
      ...
      validM3 = new Move(startingSquare, tmpLMov2, AssessValue(tmpLMov2));
  ...
  for(int n=1;n < 8;n++){
    ...
    if(!(tmpN2 > 7 || tmpN2 < 0 || tmpy4 > 7 || tmpy4 < 0)){
      ...
      validM4 = new Move(startingSquare, tmpNmov2,AssessValue(tmpNmov2));
...
private Stack getKnightMoves(int x, int y, String piece){
  ...
  for(int i=0;i < 8;i++){
    ...
    Move tmpmove = new Move(startingSquare, tmp, AssessValue(tmp));
  ...
  System.exit(0);
    ...
    */
      ...
      while(!completeMoves.empty()){
        ...
        System.out.println("The "+s1.getName()+" can move from ("+s1.getXC()+", "+s1.getYC()+") to the following square: ("+s2.getXC()+", "+s2.getYC()+" "+tmpMove.getValue());
       ...
       if(l instanceof JLabel){
         ...
         pieces = (landingPoint.getYC() == 7 && startingPoint.getName().contains("WhitePawn")) ? new JLabel( new ImageIcon("WhiteQueen.png")) : new JLabel( new ImageIcon(startingPoint.getName()+".png") );
